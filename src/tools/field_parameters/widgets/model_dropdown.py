...
Custom themed dropdown for selecting Power BI models.
...
class ModelSelectorDropdown:
    """
    A custom themed dropdown for selecting Power BI models.
    Features:
    - Themed background and borders
    - Hover effects on items
    - Click-away to close
    - Theme change support
    - Power BI icon for each model item
    """
    ...
    def __init__(
        ...
        Args:
            ...
            on_selection_changed: Callback when selection changes, receives display_name
        ...
        # State
        self._models: List[Dict] = []  # List of model info dicts
    ...
    def _load_icons(self):
        """Load Power BI icon for dropdown items."""
        if not PIL_AVAILABLE:
            return

        icons_dir = Path(__file__).parent.parent.parent.parent / "assets" / "Tool Icons"
        svg_path = icons_dir / "Power-BI.svg"
        png_path = icons_dir / "Power-BI.png"
        ...
        try:
            ...
            if img:
                ...
                self._pbi_icon = ImageTk.PhotoImage(img)
        except Exception as e:
            self._logger.debug(f"Failed to load Power BI icon: {e}")
    ...
    def _open_dropdown(self):
        ...
        if not self._models:
            ...
            # Create item for each model
            for model in self._models:
                display_name = model.get('display_name', str(model))
                ...
                if self._pbi_icon:
                    ...
                    icon_lbl._model = display_name
                    icon_lbl.bind('<Button-1>', lambda e, n=display_name: self._select_item(n))
                ...
                text_lbl._model = display_name
                text_lbl.bind('<Button-1>', lambda e, n=display_name: self._select_item(n))
                ...
                # Bind frame events
                item_frame.bind('<Button-1>', lambda e, n=display_name: self._select_item(n))
    ...
    def _select_item(self, display_name: str):
        """Handle item selection."""
        self._selected_value = display_name
        self._update_display()
        self._close_dropdown()

        if self._on_selection_changed:
            self._on_selection_changed(display_name)
    ...
    def set_models(self, models: List[Dict]):
        ...
        Args:
            models: List of model info dicts with 'display_name' key
        """
        self._models = models
        # If currently selected model is no longer available, clear selection
        display_names = [m.get('display_name', '') for m in models]
        if self._selected_value and self._selected_value not in display_names:
            self._selected_value = ""
            self._update_display()

    def set_selection(self, display_name: str):
        """Set the currently selected model by display name."""
        self._selected_value = display_name
        self._update_display()
    ...
    def clear(self):
        """Clear the selection and models list."""
        self._models = []
        self._selected_value = ""
        self._update_display()
