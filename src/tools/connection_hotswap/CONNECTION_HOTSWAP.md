...
> **Professional-grade connection management for Power BI development workflows**

## Overview

Connection Hot-Swap is a powerful tool for dynamically switching Power BI data source connections between environments without republishing reports. It enables seamless transitions between development, testing, and production datasets - whether local Power BI Desktop models or cloud-hosted Power BI Service datasets.
...
- Exact restoration to original cloud format
- No loss of GUID or endpoint information
- Seamless round-trip swapping
...
- Old presets without fingerprints still work
- Validation is skipped when no fingerprint exists
- New presets automatically include fingerprints when saved
...
**Storage:**
- Saved per-model using model hash identifier
- Persisted in `last_configs` section of preset JSON
- Includes source server, database, workspace name, and friendly name
...
**Example Flow:**
```
1. Open model connected to "Production Dataset"
2. Tool saves this as the starting configuration
3. Swap to "Development Dataset" for testing
4. Click APPLY TARGET to restore "Production Dataset"
5. Ready mappings auto-selected - click SWAP SELECTED
```
...
- Flat preset list → Scoped organization
- Added model_hash for MODEL presets
- Last Config tracking added
...
class WorkspaceInfo:
    ...
    def xmla_endpoint(self) -> str:
        return f"powerbi://api.powerbi.com/v1.0/myorg/{quote(self.name)}"
```

### Dataset Search

- Search across all accessible workspaces
- Results include workspace name
- Click to select as swap target

### Manual Entry

For datasets not discoverable via API:
- Direct XMLA endpoint URL entry
...
- Premium/PPU/Fabric: Both "Semantic Model" and "XMLA Endpoint" options available (no tooltip)
- Pro workspace: "XMLA Endpoint" option disabled with tooltip explaining the limitation (only shows when disabled)
- Automatic fallback to Semantic Model connector when XMLA unavailable

**Detection Methods:**
1. Workspace capacity type from API response
2. XMLA connection attempt during perspective loading
3. Permission/access error handling
...
- Connection format: `powerbi://api.powerbi.com/v1.0/myorg/{workspace}`

**Automatic Selection (Perspectives)**
- When a perspective is specified, the tool automatically selects the appropriate connection type:
  - Premium/PPU/Fabric: Uses XMLA endpoint (`analysisServicesDatabaseLive`)
  - Pro workspace: Uses hybrid format (`pbiServiceXmlaStyleLive`) with special connection structure
...
```
Note: This hybrid format uses XMLA-style connection string but with `pbiServiceXmlaStyleLive` type, enabling perspective support on Pro workspaces.
...
- **APPLY TARGET Button**: Renamed from "LAST CONFIG" - restores original target connections after swapping
  - Automatically saves starting configuration when scanning a model
  - One-click restoration to original connection state
  - Auto-selects ready mappings after applying for immediate swap
- **Cloud Browser Dialog Improvements**:
  - Default selection of "(All Workspaces)" when opening
  - Optimized panel widths for better usability
  - XMLA Endpoint option automatically disabled for Pro workspaces with explanatory tooltip
  - Capacity indicators for Premium/PPU workspaces in workspace list
- **XMLA Access Detection**:
  - Automatic detection of Pro workspaces (no XMLA endpoint support)
  - Visual feedback when XMLA option is unavailable
  - Tooltip explains: "XMLA endpoint not available for Pro workspaces. Requires Premium, PPU, or Fabric capacity."
- **UI Polish**:
  - SVG-based radio buttons with proper disabled states
  - Consistent tooltip styling across dialogs
  - Local model selector dialog height increased for better visibility
...
- Source perspective name display in connection details
- Improved thin report perspective handling
...
- Generic connection fallback with warning message
...
Part of the AE Power BI Multi-Tool suite.
