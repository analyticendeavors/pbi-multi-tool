...
class MiddleOutLayoutEngine:
    ...
    def generate_middle_out_layout(self, canvas_width: int = 1400, canvas_height: int = 900,
                                     filter_tables: List[str] = None) -> List[Dict[str, Any]]:
        ...
        Args:
            ...
            filter_tables: Optional list of table names to include (filters to only these tables)
        """
        ...
        # Filter to only tables in this diagram if specified
        if filter_tables:
            original_count = len(table_names)
            table_names = [t for t in table_names if t in filter_tables]
            logger.info(f"Filtered tables: {len(table_names)} of {original_count} (diagram has {len(filter_tables)} tables)")
    ...
    def apply_middle_out_layout(self, canvas_width: int = 1400, canvas_height: int = 900,
                              save_changes: bool = True, expand_all: bool = False,
                              diagram_index: int = 0, filter_tables: List[str] = None) -> Dict[str, Any]:
        ...
        Args:
            ...
            expand_all: Whether to expand all table nodes
            diagram_index: Index of the diagram to optimize (default: 0)
            filter_tables: Optional list of table names to include (only repositions these tables)
        """
        try:
            ...
            # Generate new positions using universal layout (filtered to diagram tables if specified)
            new_positions = self.generate_middle_out_layout(canvas_width, canvas_height, filter_tables=filter_tables)
            ...
            if current_layout and 'diagrams' in current_layout:
                ...
                if new_layout_data['diagrams'] and diagram_index < len(new_layout_data['diagrams']):
                    new_layout_data['diagrams'][diagram_index]['nodes'] = enhanced_nodes
                    new_layout_data['diagrams'][diagram_index]['hideKeyFieldsWhenCollapsed'] = False
            else:
                new_layout_data = {
                    "version": "1.1.0",
                    "diagrams": [
