...
class EnhancedBaseExternalTool(ABC):
    ...
    def create_secure_ui_base(self, themename: str = "cyborg") -> tk.Tk:
        """Create secure UI base with modern ttkbootstrap styling"""
        # Use ttkbootstrap for modern UI if available
    ...
    def run_with_tool_manager(self, tool_manager: ToolManager):
        ...
        try:
            ...
            # Check registration before showing main UI
            from core.registration import RegistrationManager
            reg_manager = RegistrationManager.get_instance()
            if not reg_manager.is_registered():
                # Create temporary root for registration dialog
                temp_root = tk.Tk()
                temp_root.withdraw()  # Hidden root

                from core.ui.registration_dialog import RegistrationDialog
                dialog = RegistrationDialog(temp_root, reg_manager, self.config.version if hasattr(self, 'config') else "")
                success = dialog.show()

                temp_root.destroy()

                if not success:
                    self.logger.log_operation("Registration cancelled or failed - exiting")
                    sys.exit(0)

                self.logger.log_operation("Registration completed successfully")
